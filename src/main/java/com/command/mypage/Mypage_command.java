...
public class Mypage_command implements Command {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			arr1=new Mypage_DAO().selectBySQL_withSignal(Mypage_DAO.SELECT_6_MY_POSTS,1,1);
			...
			count1=((Count_DTO)new Count_DAO().selectBySQL(Mypage_DAO.SELECT_MY_POSTS_CNT, 1)[0]).getCount();
			request.setAttribute("mypost_board_cnt", count1);
			
			//내가 작성한 댓글
			arr2=new Mypage_DAO().selectBySQL_withSignal(Mypage_DAO.SELECT_6_MY_COMMENTS,2,1);
			System.out.println("서버 확인-내가작성한댓글: "+arr2);
//			for(DTO v : arr2) {
//				System.out.println(((Post_DTO)v).getNickname());
//			}
			request.setAttribute("mycomment_board", arr2);
			count2=((Count_DTO)new Count_DAO().selectBySQL(Mypage_DAO.SELECT_MY_COMMENTS_CNT, 1)[0]).getCount();			
			request.setAttribute("mycomment_board_cnt", count2);
			
			//공감한 게시글
			arr3=new Mypage_DAO().selectBySQL_withSignal(Mypage_DAO.SELECT_6_MY_EMPATHIZE,3,1);
			request.setAttribute("myempathize_board", arr3);
			count3=((Count_DTO)new Count_DAO().selectBySQL(Mypage_DAO.SELECT_MY_EMPATHIZE_CNT, 1)[0]).getCount();
